...
class FilesController {
  ...
  static async postUpload(request, response) {
    ...
    const existinUser = await userUtils.getUser({
      _id: ObjectId(userId),
    });

    if (!existinUser) return response.status(401).send({ error: 'Unauthorized' });
  ...
  static async getFile(request, response) {
    ...
    const fileById = await fileUtils.getFile({
      _id: ObjectId(fileId),
    });

    if (!fileById || !fileUtils.isOwnerAndPublic(fileById, userId)) { return response.status(404).send({ error: 'Not found' }); }
    ...
    const { error, code, data } = await fileUtils.getFileData(fileById, size);

    if (error) return response.status(code).send({ error });

    const typeOfMime = mime.contentType(fileById.name);

    response.setHeader('Content-Type', typeOfMime);
